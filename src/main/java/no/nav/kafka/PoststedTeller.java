...
public class PoststedTeller {

    private final HashMap<String, Integer> count = new HashMap<>();


    public void lagre(final Poststed poststed) {
    ...
    public void skrivPoststederEtterAntallTreff() {
        HashMap<String, Integer> sortertEtterAntallTreff = count
            .entrySet()
            .stream()
            .sorted((Map.Entry.<String, Integer>comparingByValue().reversed()))
            //.limit(3)
            .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (e1, e2) -> e1, LinkedHashMap::new));
        if (!sortertEtterAntallTreff.isEmpty()) {
            System.out.println(sortertEtterAntallTreff);
        }
